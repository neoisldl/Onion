...
abstract class Router_Abstract {
     ...
     * @return mixed
     */
    public abstract function dispatch($url, array $params = array());
     ...
     * @return string
     */
    public static function url($actions, $params = array()) {
        $args = func_get_args();
...
class Router extends Router_Abstract {
    ...
    protected function match($url) {
        foreach ($this->dispatch_rewrite as $re => $class) {
            if (preg_match($re, $url, $match)) {
                if (DEBUG) \Onion\logger('mvc')->debug('Match rewrite rule: '. $re);
    ...
    public function dispatch($url, array $params = array()) {
        if (DEBUG) $logger = \Onion\logger('mvc');
